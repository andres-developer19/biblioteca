...
if($usuario!="")
{
	...
	case 'Buscar Libro':
	 ...
	 if($buscar=='cota_lib'){
		 $sql2="SELECT * from lib_doc, libro where lib_doc.cota=libro.cota_lib and cota like  '%".$valor."-%' and lib_doc.status='DISPONIBLE'";}
 	 else{
		 $sql2="SELECT DISTINCT * FROM lib_doc,libro where  lib_doc.cota=libro.cota_lib and $buscar='$valor' and lib_doc.status='DISPONIBLE'";
		 }
	  //$sql2="SELECT * FROM lib_doc, libro WHERE $buscar='$valor'";
...
<?php
	...
	?><script>window.location.href='index.php?pag=5';</script><?php
...
<legend style="text-align:center"><B>PRESTAMO DE LIBRO</B><br /></legend>
...
<option value="cota_lib"  <?php if($buscar=='cota_lib'){echo 'selected';}?>>Codigo</option>
<option value="nombre" <?php if($buscar=='nombre'){echo 'selected';}?>>Titulo</option>
<option value="autor" <?php if($buscar=='autor'){echo 'selected';}?>>Autor</option>
<option value="asignatura" <?php if($buscar=='asignatura'){echo 'selected';}?>>Asignatura</option>
<option value="grado" <?php if($buscar=='grado'){echo 'selected';}?>>Grado</option>
</select>
...
include("funciones/conexion.php");
  ...
  if($buscar=='cota_lib'){$sql="SELECT DISTINCT SUBSTR(cota ,1,3) as lista from lib_doc where tipo='LIBRO'";}
  else{$sql="SELECT DISTINCT $buscar as lista FROM lib_doc,libro where lib_doc.cota=libro.cota_lib";}
  $resultado=mysql_db_query($bd,$sql);
  echo mysql_error();
...
<button  type="submit" value="Buscar Libro" name="boton" onClick="valida_envia()"><img src="imagenes/buscar.png" title="Buscar"/></button>
</td>
</tr>
</table>
...
<legend>Listado</legend>
 ...
 <tr>
     ...
     <td align="center">
      <?php echo $contenido2 ['autor'];?>
     </td>
     <td align="center">
      <?php echo $contenido2 ['asignatura'];?>
     </td>
    <td align="center">
      <?php echo $contenido2 ['grado'];?>
     </td>
     <td align="center">
